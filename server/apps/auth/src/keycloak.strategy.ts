...
export class KeycloakStrategy extends PassportStrategy(Strategy, 'keycloak') { // Définit une stratégie Passport appelée 'keycloak'.
  constructor() {
    super({
      ...
      audience: ['public-client', 'account'], // Définit les audiences acceptées dans le JWT. À la place de XXX, il faut mettre le titre du Clients que nous avons déjà créé dans Keycloak
      issuer: 'http://localhost:8080/realms/XXX', // Identifie l'émetteur attendu du JWT. À la place de XXX, il faut mettre le nom du Realm
      ...
      secretOrKeyProvider: passportJwtSecret({
        ...
        jwksUri: 'http://localhost:8080/realms/XXX/protocol/openid-connect/certs' // URL pour récupérer les clés publiques JWKS de Keycloak.À la place de XXX, il faut mettre le nom du Realm
      }),
    });
  }
